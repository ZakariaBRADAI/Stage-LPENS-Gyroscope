...
###
def the_dd_X(t, Epp, the, phi, psi, the_d, phi_d, psi_d, p_psi, J1_, w, p, K):
    the_dd_free = the_dd_Free(t, Epp, the, phi, psi, the_d, phi_d, psi_d, p_psi, J1_)
    the_dd_forced = K * (w**2) * de.cos(w*t+p) * de.sin(phi) * de.cos(the) / J1_
    the_dd = the_dd_free + the_dd_forced
    return the_dd


def phi_dd_X(t, the, phi, psi, the_d, phi_d, psi_d, p_psi, J1_, w, p, K):
    phi_dd_free = phi_dd_Free(t, the, phi, psi, the_d, phi_d, psi_d, p_psi, J1_)
    phi_dd_forced = K * (w**2) * de.cos(w*t+p) * de.cos(phi) / (J1_ * de.sin(the))
    phi_dd = phi_dd_free + phi_dd_forced
    return phi_dd


def psi_dd_X(t, the, phi, psi, the_d, phi_d, psi_d, p_psi, J1_, w, p, K):
    psi_dd_free = psi_dd_Free(t, the, phi, psi, the_d, phi_d, psi_d, p_psi, J1_)
    psi_dd_forced = - K * (w**2) * de.cos(w*t+p) * de.cos(phi) / (J1_ * de.tan(the))
...
def Gyro_Mvt_Forced_X(u, params, t):
    the, the_d, phi, phi_d, psi, psi_d = u
    ...
    return [
            the_d,
            the_dd_X(t, Epp, the, phi, psi, the_d, phi_d, psi_d, p_psi, J1_, w, p, K),
            phi_d,
            phi_dd_X(t, the, phi, psi, the_d, phi_d, psi_d, p_psi, J1_, w, p, K),
            psi_d,
            psi_dd_X(t, the, phi, psi, the_d, phi_d, psi_d, p_psi, J1_, w, p, K),
            ]
