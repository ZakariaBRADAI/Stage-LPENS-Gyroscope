...
def Gyro_Solver(tspan, CI, params, forcing='Free', precision='HIGH'):

    #the, the_d, phi, phi_d, psi, psi_d = u
    
    g, m, h, J1, J3, x0, Phi, w = params
    ...
    match forcing:
        ...
        case 'X':
            prob = de.ODEProblem(Gyro_Mvt_Forced_X, CI, tspan, adapt_params)
    ...
    match precision:
        case 'LOW':
            sol = de.solve(fastprob, de.RadauIIA5(), abstol=10e-6, reltol = 1e-13, maxiters=np.inf)
        case 'HIGH':
            sol = de.solve(fastprob, de.RadauIIA5(), abstol = 1e-8, reltol = 1e-5, maxiters=np.inf)
            #sol = de.solve(fastprob, de.RadauIIA5(), abstol = 1e-9, reltol = 1e-9, maxiters=np.inf)
            #sol = de.solve(fastprob, de.AutoVern7(de.Rodas5()), reltol = 1e-9, maxiters=np.inf, alg_hints = ['stiff'])
...
def Cobra(dt1, dt2, dt3, CI, params, forcing='XY', precision = 'HIGH'):
    '''Sam's work.'''
    ...
    t1, the1, phi1, psi1, the_d1, phi_d1, psi_d1 = Gyro_Solver(tspan_1, CI, params, forcing=forcing, precision=precision)

    CI1 = [the1[-1], the_d1[-1], phi1[-1], phi_d1[-1], psi1[-1], psi_d1[-1]]
    t2, the2, phi2, psi2, the_d2, phi_d2, psi_d2 = Gyro_Solver(tspan_2, CI1, params, forcing='Free', precision=precision)

    CI2 = [the2[-1], the_d2[-1], phi2[-1], phi_d2[-1], psi2[-1], psi_d2[-1]]
    params2 = params
    params[-2] = 0
    t3, the3, phi3, psi3, the_d3, phi_d3, psi_d3 = Gyro_Solver(tspan_3, CI2, params2, forcing=forcing, precision=precision)
